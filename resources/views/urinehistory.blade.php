...
<!--<li><a href="#3">Request Lab Report</a></li>-->
        ...
        <div class="container">
		<form action="/search02" method="POST" role="search">
			{{ csrf_field() }}
			<div class="input-group">
				<input type="text" class="form-control" name="p"
					placeholder="Search Patient Lab Report"> <span class="input-group-btn">
		...
		<div class="container">
			@if(isset($details))
			<p> The Search results for Lab Report No <b> {{ $query }} </b> are :</p>
			<h2>Urine Report details</h2>
			<table class="table table-striped">
				...
				<tbody>
					@foreach($details as $ulab)
				<td>{{$ulab->lab_id}}</td>
				<td>{{$ulab->id}}</td>
				<td>{{$ulab->first_name}}</td>
				<td>{{$ulab->created_at}}</td>
				<td>{{$ulab->color}} </td>
				<td>{{$ulab->appearance}}  </td>
				<td>{{$ulab->ph}} </td>
				<td>{{$ulab->glucose}}  </td>
				<td>{{$ulab->wpc}}  </td>
				<td>{{$ulab->rbc}}  </td>
